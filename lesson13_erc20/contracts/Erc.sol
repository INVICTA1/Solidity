...
contract ERC20 is IERC20 {
    ...
    mapping(address => mapping(address => uint)) allowances;// Keep the information that we can withdraw tokens from the wallet to another wallet
    ...
    constructor(string memory name_, string memory symbol_, uint initialSupply, address shop){
        ...
        // Create tokens(numbers is initialSupply) and send to address(shom)
    }
    ...
    function balanceOf(address accaunt) public view returns (uint){
        return balances[accaunt];
    }
    ...
    function transferFrom(address sender, address recipient, uint amount) public enoughTokens(sender, amount) {
        ...
        // error if will be negative number
...
contract MShop {
    ...
    function sell(uint _amountToSell) external {
        require(_amountToSell > 0 && token.balanceOf(msg.sender) >= _amountToSell,
            "incorrect amount"
        );
