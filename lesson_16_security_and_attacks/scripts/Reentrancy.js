...
async function main() {
    ...
    const txBid = await auction.bid({value: ethers.utils.parseEther("5")})
    await txBid.wait()

    const txBid1 = await auction.connect(bidder2).bid({value: ethers.utils.parseEther("6.0")})
    await txBid1.wait()

    const txBid2 = await auction.connect(hacker).bid({value: ethers.utils.parseEther("5.0")})
    await txBid2.wait()

    const txBid3 = await attack.connect(hacker).proxyBid({value: ethers.utils.parseEther("1.0")})
    await txBid3.wait()
