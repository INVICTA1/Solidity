...
contract ReentrancyAuction {
    ...
    // pull
//    function refund() external noReentrancy  {// add noReentrancy modifier to protect a reentrancy attack
    function refund() external {
        uint refundAmount = bidders[msg.sender];

        if (refundAmount > 0) {
//            bidders[msg.sender] = 0;// - change logic also protect from reentrancy attack
